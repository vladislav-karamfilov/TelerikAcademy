...
namespace GeometryAPI
{
    ...
    public class Circle : Figure, IAreaMeasurable, IFlat
    {
        ...
        public Vector3D GetNormal()
        {
            // Because the circle is flat and it's in the plane XY which is perpendicular to the vector (0, 0, 1) 
    ...
    {
        public Vector3D BottomBase { get; private set; }
        public Vector3D TopBase { get; private set; }
        public double Radius { get; private set; }
        ...
        {
            Vector3D heightVector = this.TopBase - this.BottomBase;
            return heightVector.Magnitude;
        }
    }
    ...
    public class ExtendedFigureController : FigureController
    {
        public override void ExecuteFigureCreationCommand(string[] splitFigString)
        {
            switch (splitFigString[0])
            {
                case "circle":
                    ...
                    this.currentFigure = new Circle(center, circleRadius);
                    break;
                case "cylinder":
                    Vector3D topBase = Vector3D.Parse(splitFigString[1]);
                    Vector3D bottomBase = Vector3D.Parse(splitFigString[2]);
                    double cylinderRadius = double.Parse(splitFigString[3]);
                    this.currentFigure = new Cylinder(bottomBase, topBase, cylinderRadius);
                    break;
            }
        ...
        protected override void ExecuteFigureInstanceCommand(string[] splitCommand)
        {
            ...
            {
                case "area":
                    IAreaMeasurable figureWithArea = this.currentFigure as IAreaMeasurable;
                    if (figureWithArea != null)
                    {
                        Console.WriteLine("{0:0.00}", figureWithArea.GetArea());
                    }
                    else
                    {
                    ...
                    break;
                case "volume":
                    IVolumeMeasurable figureWithVolume = this.currentFigure as IVolumeMeasurable;
                    if (figureWithVolume != null)
                    {
                        Console.WriteLine("{0:0.00}",  figureWithVolume.GetVolume());
                    }
                    else
                    {
                        Console.WriteLine("undefined");
