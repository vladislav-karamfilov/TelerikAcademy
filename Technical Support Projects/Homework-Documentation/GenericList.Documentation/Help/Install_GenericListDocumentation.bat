...
HelpLibraryManagerLauncher.exe /product "VS" /version "100" /locale en-us /uninstall /silent /vendor "Vendor Name" /productName "Generic List Help Documentation" /mediaBookList "Generic List Help Documentation" > NUL
...
REM build time.
IF EXIST "GenericListDocumentation.msha" COPY /Y "GenericListDocumentation.msha" HelpContentSetup.msha
...
REM Uninstall first in case it is already there.  If not, it won't install below.  We'll ignore any error output
REM by redirecting it to NUL.
HelpLibraryManagerLauncher.exe /viewerVersion 2.0 /catalogName VisualStudio11 /locale en-us /wait 0 /operation uninstall /vendor "Vendor Name" /productName "Generic List Help Documentation" /bookList "Generic List Help Documentation" > NUL

REM Install the new content.
HelpLibraryManagerLauncher.exe /viewerVersion 2.0 /catalogName VisualStudio11 /locale en-us /wait 0 /operation install /sourceUri "%CD%\GenericListDocumentation.msha"
